...
class SSP {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    ...
                    $index = $column['dt'];
                    if(preg_match("|.* as (.*)|",$index,$match))
                    {
                        $index = isset($match[1]) ? $match[1] : $match[0];
                    }
                    $row[ $index ] = $data[$i][ $index ];
                }
            }
            $out[] = $row;
        }
     ...
     */
    static function limit ( $request, $columns = NULL )
    {
        $limit = '';
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    ...
                    preg_match("|(.*) as .*|",$column['db'],$match);
                    $newcolumn = isset($match[1]) ? $match[1] : $column['db'];

                    $tmp = explode('.', $newcolumn);
                    $c = isset($tmp[1]) ? $tmp[0]."`.`".$tmp[1] : $tmp[0];

                    $globalSearch[] = "`".$c."` LIKE ".$binding;
                }
            }
        }
        ...
        if ( isset( $request['columns'] ) ) {
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' &&
                    ...
                    $tmp = explode('.', $column['db']);
                    $c = isset($tmp[1]) ? $tmp[0]."`.`".$tmp[1] : $tmp[0];

                    $columnSearch[] = "`".$c."` LIKE ".$binding;
                }
            }
        }
    ...
    static function simpleJoin ( $request, $conn, $table, $primaryKey, $columns, $join, $leftJoint = "" )
    {
        ...
        /*echo "SELECT SQL_CALC_FOUND_ROWS ".implode(", ", $pluck)."
             FROM $fromTables $leftJoint
             $where
             $order
             $limit";*/
        $data = self::sql_exec( $db, $bindings,
            "SELECT SQL_CALC_FOUND_ROWS ".implode(", ", $pluck)."
			 FROM $fromTables $leftJoint
			 $where
			 $order
			 $limit"
        ...
        $resTotalLength = self::sql_exec( $db,
            "SELECT COUNT(*)
			 FROM   " . $fromTables . ((!empty($join)) ? " WHERE " . $join : "")
        );
        $recordsTotal = $resTotalLength[0][0];
        /*
         * Output
