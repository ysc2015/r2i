...
$form["infozone_site_origine"]["fields"] = array(
    array(
        ...
        "ref_select" => "",
        "select_func" => "",
        "value" => ($projet !== NULL?$projet->code_site_origine:"")
    ),
    array(
        "id" => "type_so",
        "field_name" => "type",
        ...
        "select_func" => "",
        "value" => ($projet !== NULL?$projet->type_site_origine:"")
    ),
    array(
        "id" => "auto_adduction",
...
function build_user_form($form_name,$object=NULL) {
    ...
    foreach($form[$form_name]["fields"] as $key => $val)  {
        ...
        switch($val["input_type"]) {
            case "text" : {
                $html .="<input class=\"form-control\" type=\"text\" id=\"{$val["id"]}\" name=\"{$val["id"]}\" {$val["readonly"]} ".(isset($val["value"])&& $val["value"]!="" ? "value=\"{$val["value"]}\"":($object !== NULL?"value=\"{$object->$val["field_name"]}\"":"")).">";break;
            }
            case "number" : {
                $html .="<input class=\"form-control\" type=\"number\" id=\"{$val["id"]}\" name=\"{$val["id"]}\" {$val["readonly"]} ".(isset($val["value"])&& $val["value"]!="" ? "value=\"{$val["value"]}\"":($object !== NULL?"value=\"{$object->$val["field_name"]}\"":"")).">";break;
            }
            case "date" : {
                $html .="<input class=\"form-control\" type=\"date\" id=\"{$val["id"]}\" name=\"{$val["id"]}\" {$val["readonly"]} ".(isset($val["value"])&& $val["value"]!="" ? "value=\"{$val["value"]}\"":($object !== NULL?"value=\"{$object->$val["field_name"]}\"":"")).">";break;
            }
            case "textarea" : {
                $html .="<textarea class=\"form-control\" id=\"{$val["id"]}\" name=\"{$val["id"]}\" rows=\"3\" {$val["readonly"]}>".(isset($val["value"])&& $val["value"]!="" ? "value=\"{$val["value"]}\"":($object !== NULL?"value=\"{$object->$val["field_name"]}\"":""))."</textarea>";
                break;
            }

            case "select" : {
                $html .= "<select class=\"form-control\" id=\"{$val["id"]}\" name=\"{$val["id"]}\" {$val["disabled"]}>";//TODO check readonly attr for selectbox
                ...
                if($val["select_func"] !== "") {
                    ...
                    foreach($results as $result) {
                        $html .= "<option value=\"{$result->{"id_".$val["ref_select"]}}\" ".(isset($val["value"])&&$val["value"]!=""?($val["value"] == $result->{"id_".$val["ref_select"]}?"selected":""):($object !== NULL?($object->$val["field_name"] == $result->{"id_".$val["ref_select"]}?"selected":""):"")).">{$result->{"lib_".$val["ref_select"]}}</option>";
                    }
                }
                $html .= "</select>";
                break;
