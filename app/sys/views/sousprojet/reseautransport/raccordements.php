            ...
            <div class="form-group">
                <div class="col-md-3">
                    ...
                    <select class="form-control " id="tr_id_entreprise" name="tr_id_entreprise">
                        ...
                        foreach($results as $result) {
                            echo "<option value=\"$result->id_entreprise\" ". ($sousProjet->transportraccordement!==NULL && $sousProjet->transportraccordement->id_entreprise==$result->id_entreprise ?"selected": "")." >$result->lib_entreprise</option>";
                        }
                        ?>
                    </select>
                </div>
    ...
    var traccord_chambre_uploader_options = {
        ...
        onLoad:function(obj)
        {
            $.ajax({
                ...
                method:"POST",
                data: {id_objet:get('idsousprojet'),type_objet:'transport_raccord_chambre'},
                dataType: "json",
    ...
    var traccord_pboite_uploader_options = {
        ...
        onLoad:function(obj)
        {
            $.ajax({
                ...
                url: "api/file/load.php",
                method:"POST",
                data: {id_objet:get('idsousprojet'),type_objet:'transport_racoord_pboite'},
                dataType: "json",
                success: function(data)
                {
                    for(var i=0;i<data.length;i++)
