            ...
            <div class="form-group">
                <div class="col-md-3">
                    ...
                    <select class="form-control " id="dr_id_entreprise" name="dr_id_entreprise">
                        ...
                        foreach($results as $result) {
                            echo "<option value=\"$result->id_entreprise\" ". ($sousProjet->distributionraccordement!==NULL && $sousProjet->distributionraccordement->id_entreprise==$result->id_entreprise ?"selected": "")." >$result->lib_entreprise</option>";
                        }
                        ?>
                    </select>
                </div>
    ...
    var draccord_chambre_uploader_options = {
        ...
        onLoad:function(obj)
        {
            $.ajax({
                ...
                method:"POST",
                data: {id_objet:get('idsousprojet'),type_objet:'distribution_raccord_chambre'},
                dataType: "json",
    ...
    var draccord_pboite_uploader_options = {
        ...
        onLoad:function(obj)
        {
            $.ajax({
                ...
                url: "api/file/load.php",
                method:"POST",
                data: {id_objet:get('idsousprojet'),type_objet:'distribution_racoord_pboite'},
                dataType: "json",
                success: function(data)
                {
                    for(var i=0;i<data.length;i++)
