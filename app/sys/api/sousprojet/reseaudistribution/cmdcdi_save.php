...
if($insert == true && $err == 0){
    if($stm->execute()){
        ...
        if($mailaction_new && (( $mailaction_entite !=null && $mailaction_entite->intervenant_be  != $dcc_intervenant_be ) || ($dcc_intervenant_be!="" && $mailaction_entite==null))  ){
            ...
            $mailaction_html = get_content_html_mail_by_type($db,$sousProjet->projet->nro->lib_nro."-".$sousProjet->zone,'CDI','Commande',2,'','','','','','','',$sousProjet->projet->id_chef_projet);
            $mailaction_object = $mailaction_html[1];
            $mailaction_html =  $mailaction_html[0];
        ...
        if($mailaction_new
            ...
            $mailaction_html = get_content_html_mail_by_type($db,$sousProjet->projet->nro->lib_nro."-".$sousProjet->zone,'CDI','Commande',7,'');
            $mailaction_object = $mailaction_html[1];
            $mailaction_html =  $mailaction_html[0];
