...
if(true) {
    ...
    $stm = $db->prepare("update point_bloquant_solutions_preconisees set $fieldslist where id_point_bloquant=:id_point_bloquant");
...
if($err == 0){
    if($stm->execute()){
        $message [] = "Point bloquant Solutions préconisées enregistré avec succès";
    } else {
