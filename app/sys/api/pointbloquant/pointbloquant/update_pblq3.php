...
if(true) {
    ...
    $stm = $db->prepare("update point_bloquant_moyens_mis_en_oeuvre set $fieldslist where id_point_bloquant=:id_point_bloquant");
...
if($err == 0){
    if($stm->execute()){
        $message [] = "Point bloquant Moyens mis en oeuvre enregistré avec succès";
    } else {
