...
switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT nom as id,nom as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'nom IN (' . implode(',', $ids) . ')');
    ...
    case 'update':
        ...
        $ret = DBHelper::update($tableName, 'nom=:nom_update', $post_data,array('nom_update'));
        if ($ret !== false) {
