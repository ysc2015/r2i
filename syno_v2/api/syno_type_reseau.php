...
switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT lib_type_reseau as id,lib_type_reseau as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'lib_type_reseau IN (' . implode(',', $ids) . ')');
    ...
    case 'update':
        $post_data = array(
            'lib_type_reseau_update' => $_POST['lib_type_reseau_update'],

          "lib_type_reseau" => $_POST[$lang["lib_type_reseau_id"]],);
        $ret = DBHelper::update($tableName, 'lib_type_reseau=:lib_type_reseau_update', $post_data,array('lib_type_reseau_update'));
        if ($ret !== false) {
