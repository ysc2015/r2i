<?php
$tableName = 'syno_type_chambre';
$response = array('err' => 0, 'msg' => array(), 'extra' => null);

switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT lib_type_chambre as id,lib_type_chambre as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'lib_type_chambre IN (' . implode(',', $ids) . ')');
    ...
    case 'update':
        $post_data = array(
            'lib_type_chambre_update' => $_POST['lib_type_chambre_update'],

          "lib_type_chambre" => $_POST[$lang["lib_type_chambre_id"]],);
        $ret = DBHelper::update($tableName, 'lib_type_chambre=:lib_type_chambre_update', $post_data,array('lib_type_chambre_update'));
        if ($ret !== false) {
