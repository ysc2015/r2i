...
$nbr = DBHelper::count('syno_passage');
        ...
        <div class="col-lg-12 animated fadeInRight">
            <div class="mail-box-header">
                <form class="pull-right mail-search">
                    <div class="input-group">
                        ...
                        <select id="search_by" class="form-control input-sm">
                            <option value="id_passage"><?php echo $lang["id_passage_label"]; ?></option><option value="lib_passage"><?php echo $lang["lib_passage_label"]; ?></option>
                        </select>
                    </div>
                </form>

                <h2>
                    <?php echo $lang['syno_passage_TITLE']; ?> 
                    <sup><span class="label label-info" id="nbrOf_syno_passage"><?php echo $nbr; ?></span>
                ...
                <div class="mail-tools tooltip-demo m-t-md">
                    ...
                    <button id="btn_new" data-toggle="modal" href="#modal-form" class="btn btn-primary"><i class="fa fa-plus-circle"></i> New syno_passage</button>
            ...
            <div class="mail-box-header">
                <button class="btn btn-xs btn-primary" onclick="show_hide(0)">id_passage</button> <button class="btn btn-xs btn-primary" onclick="show_hide(1)">lib_passage</button> 
            </div>

            <div class="mail-box">
                <table class="table table-bordered table-hover table-mail">
                    <thead>
                        <tr><th></th><th id="cell_0_0">id_passage</th><th id="cell_0_1">lib_passage</th><th>Action</th></tr>
                    </thead>
                    <tbody id="syno_passage_body_id">
                    </tbody>
                    <tfoot>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 b-r">
                        <h3 class="m-t-none m-b"><?php echo $lang["syno_passage_modal_title"]; ?></h3>

                        <p><?php echo $lang["syno_passage_modal_description"]; ?></p>

                        <input id="id_passage_update" type="hidden" value=""> 
                        
                        <div class="form-group">
                            <label for="<?php echo $lang["id_passage_id"]; ?>"><?php echo $lang["id_passage_label"]; ?></label> 
                            <input id="<?php echo $lang["id_passage_id"]; ?>" placeholder="<?php echo $lang["id_passage_placeholder"]; ?>" class="form-control " type="text" readonly>
                        </div>
                        <div class="form-group">
                            <label for="<?php echo $lang["lib_passage_id"]; ?>"><?php echo $lang["lib_passage_label"]; ?></label> 
                            <input id="<?php echo $lang["lib_passage_id"]; ?>" placeholder="<?php echo $lang["lib_passage_placeholder"]; ?>" class="form-control " type="text" >
                        </div>
                        <div>
...
<script type="text/javascript">
    ...
    var apiName = 'syno_passage';
    var lastResponseData = null;
    var hiddenColumns = [false,false];
    
    function getData() {
        var obj = {
                        <?php echo $lang["id_passage_id"]; ?>: valById("<?php echo $lang["id_passage_id"]; ?>"),
            <?php echo $lang["lib_passage_id"]; ?>: valById("<?php echo $lang["lib_passage_id"]; ?>"),

        };
        if($('#id_passage_update').val() !== '') {
            obj.id_passage_update = $('#id_passage_update').val();
        }
        return obj;
    }
    ...
    function checkData(data) {
        ...
        if (data.<?php echo $lang["lib_passage_id"]; ?> == "") {
            showNotification("Error", ["<?php echo $lang["lib_passage_empty_message"]; ?>"], "error");
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.err == 0) {
                    showNotification('DONE', response.msg);
                    listAPI(page);
                    var nbr = $('#nbrOf_syno_passage').text();
                    nbr++;
                    $('#nbrOf_syno_passage').text(nbr);
                }
            },
            error: function (response) {},
            complete: function () {},
            ...
            success: function (response) {
                if (response.err == 0) {
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        html += '<input value="' + response.data[i].id_passage + '" type="checkbox" class="i-checks">';
                        html += '</td>';
                        html += "<td id=\"cell_" + i + "_0\" class=\"mail-contact\">" + response.data[i].id_passage + "</td>";
                        html += "<td id=\"cell_" + i + "_1\" class=\"mail-contact\">" + response.data[i].lib_passage + "</td>";
                        ...
                        html += '</tr>';
                        $('#syno_passage_body_id').append(html);
                    ...
                    $('#nbrOf_syno_passage').text('Number : ' + response.extra.NbrQ + ' Total : ' + response.extra.NbrT);
                }
            },
            error: function (response) {},
            complete: function () {},
    ...
    function btn_edit_click(index) {
        ...
        $('#id_passage_update').val(data.id_passage);
                $("#<?php echo $lang["id_passage_id"]; ?>").val(data.id_passage);
        $("#<?php echo $lang["lib_passage_id"]; ?>").val(data.lib_passage);
