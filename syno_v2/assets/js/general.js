function showNotification2(message, type, title) {
...
function sendToApi(api, action, data,onSuccessCallBack) {
    $.ajax({
        ...
        success: function (data) {
            var date = new Date();
            if (data.err == 0) {
                showNotification2(data.msg, 'success', date.getFullYear() + '/' + date.getMonth() + '/' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds());
            } else {
                showNotification2(data.msg, 'danger', date.getFullYear() + '/' + date.getMonth() + '/' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds());
            }
        ...
        error: function (data) {
            showNotification2('ERROR ' + data, 'danger');
        }
    });
}
...
function deleteImage(imageID) {
    sendToApi('photos', 'delete', { id: imageID },imageUploader);
...
function imageUploader(chambreID) {
    ...
    $.ajax({
        ...
        beforeSend : function () {
            $('#photos_area').html('<img src="assets/img/gear.gif" id="loading_gif" width="32px" height="32px" />');
        },
        error: function (data) {
        }
    });
