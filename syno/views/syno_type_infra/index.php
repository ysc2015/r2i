...
$nbr = DBHelper::count('syno_type_infra');
        ...
        <div class="col-lg-12 animated fadeInRight">
            <div class="mail-box-header">
                <form class="pull-right mail-search">
                    <div class="input-group">
                        ...
                        <select id="search_by" class="form-control input-sm">
                            <option value="id_type_infra"><?php echo $lang["id_type_infra_label"]; ?></option><option value="lib_type_infra"><?php echo $lang["lib_type_infra_label"]; ?></option>
                        </select>
                    </div>
                </form>

                <h2>
                    <?php echo $lang['syno_type_infra_TITLE']; ?> 
                    <sup><span class="label label-info" id="nbrOf_syno_type_infra"><?php echo $nbr; ?></span>
                ...
                <div class="mail-tools tooltip-demo m-t-md">
                    ...
                    <button id="btn_new" data-toggle="modal" href="#modal-form" class="btn btn-primary"><i class="fa fa-plus-circle"></i> New syno_type_infra</button>
            ...
            <div class="mail-box-header">
                <button class="btn btn-xs btn-primary" onclick="show_hide(0)">id_type_infra</button> <button class="btn btn-xs btn-primary" onclick="show_hide(1)">lib_type_infra</button> 
            </div>

            <div class="mail-box">
                <table class="table table-bordered table-hover table-mail">
                    <thead>
                        <tr><th></th><th id="cell_0_0">id_type_infra</th><th id="cell_0_1">lib_type_infra</th><th>Action</th></tr>
                    </thead>
                    <tbody id="syno_type_infra_body_id">
                    </tbody>
                    <tfoot>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 b-r">
                        <h3 class="m-t-none m-b"><?php echo $lang["syno_type_infra_modal_title"]; ?></h3>

                        <p><?php echo $lang["syno_type_infra_modal_description"]; ?></p>

                        <input id="id_admin" type="hidden" value=""> 
                        
                        <div class="form-group">
                            <label for="<?php echo $lang["id_type_infra_id"]; ?>"><?php echo $lang["id_type_infra_label"]; ?></label> 
                            <input id="<?php echo $lang["id_type_infra_id"]; ?>" placeholder="<?php echo $lang["id_type_infra_placeholder"]; ?>" class="form-control " type="text" readonly>
                        </div>
                        <div class="form-group">
                            <label for="<?php echo $lang["lib_type_infra_id"]; ?>"><?php echo $lang["lib_type_infra_label"]; ?></label> 
                            <input id="<?php echo $lang["lib_type_infra_id"]; ?>" placeholder="<?php echo $lang["lib_type_infra_placeholder"]; ?>" class="form-control " type="text" >
                        </div>
                        <div>
...
<script type="text/javascript">
    ...
    var apiName = 'syno_type_infra';
    var lastResponseData = null;
    var hiddenColumns = [false,false];
    
    function getData() {
        var obj = {
                        <?php echo $lang["id_type_infra_id"]; ?>: valById("<?php echo $lang["id_type_infra_id"]; ?>"),
            <?php echo $lang["lib_type_infra_id"]; ?>: valById("<?php echo $lang["lib_type_infra_id"]; ?>"),
    ...
    function checkData(data) {
        ...
        if (data.<?php echo $lang["lib_type_infra_id"]; ?> == "") {
            showNotification("Error", ["<?php echo $lang["lib_type_infra_empty_message"]; ?>"], "error");
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.err == 0) {
                    showNotification('DONE', response.msg);
                    listAPI(page);
                    var nbr = $('#nbrOf_syno_type_infra').text();
                    nbr++;
                    $('#nbrOf_syno_type_infra').text(nbr);
                }
            },
            error: function (response) {},
            complete: function () {},
            ...
            success: function (response) {
                if (response.err == 0) {
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        html += '<input value="' + response.data[i].id_type_infra + '" type="checkbox" class="i-checks">';
                        html += '</td>';
                        html += "<td id=\"cell_" + i + "_0\" class=\"mail-contact\">" + response.data[i].id_type_infra + "</td>";
                        html += "<td id=\"cell_" + i + "_1\" class=\"mail-contact\">" + response.data[i].lib_type_infra + "</td>";
                        ...
                        html += '</tr>';
                        $('#syno_type_infra_body_id').append(html);
                    ...
                    $('#nbrOf_syno_type_infra').text('Number : ' + response.extra.NbrQ + ' Total : ' + response.extra.NbrT);
                }
            },
            error: function (response) {},
            complete: function () {},
    ...
    function btn_edit_click(index) {
        ...
        $('#btn_save').attr('data-api', 'update');
                $("#<?php echo $lang["id_type_infra_id"]; ?>").val(data.id_type_infra);
        $("#<?php echo $lang["lib_type_infra_id"]; ?>").val(data.lib_type_infra);
