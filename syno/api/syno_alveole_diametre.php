...
switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT id_alveole_diametre as id,val_alveole_diametre as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'id_alveole_diametre IN (' . implode(',', $ids) . ')');
    ...
    case 'update':
        $admin = array(
          "id_alveole_diametre" => $_POST[$lang["id_alveole_diametre_id"]],
          "val_alveole_diametre" => $_POST[$lang["val_alveole_diametre_id"]],);
        $ret = DBHelper::update($tableName, 'id_alveole_diametre=:id_alveole_diametre', $admin);
        if ($ret !== false) {
