<?php
$tableName = 'syno_type_infra';
$response = array('err' => 0, 'msg' => array(), 'extra' => null);

switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT id_type_infra as id,lib_type_infra as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'id_type_infra IN (' . implode(',', $ids) . ')');
    ...
    case 'add':
        $data = array(
          "lib_type_infra" => $_POST[$lang["lib_type_infra_id"]],);
    ...
    case 'update':
        $admin = array(
          "id_type_infra" => $_POST[$lang["id_type_infra_id"]],
          "lib_type_infra" => $_POST[$lang["lib_type_infra_id"]],);
        $ret = DBHelper::update($tableName, 'id_type_infra=:id_type_infra', $admin);
        if ($ret !== false) {
