...
switch ($action) {
    case 'listForSelect':
        $stmt = $pdo->query('SELECT id_diametre as id,valeur_diametre as lib FROM ' . $tableName);
    ...
    case 'delete':
        ...
        $ret = DBHelper::delete($tableName, 'id_diametre IN (' . implode(',', $ids) . ')');
    ...
    case 'update':
        $admin = array(
          "id_diametre" => $_POST[$lang["id_diametre_id"]],
          "valeur_diametre" => $_POST[$lang["valeur_diametre_id"]],);
        $ret = DBHelper::update($tableName, 'id_diametre=:id_diametre', $admin);
        if ($ret !== false) {
