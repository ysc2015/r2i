...
$nbr = DBHelper::count('syno_diametre');
        ...
        <div class="col-lg-12 animated fadeInRight">
            <div class="mail-box-header">
                <form class="pull-right mail-search">
                    <div class="input-group">
                        ...
                        <select id="search_by" class="form-control input-sm">
                            <option value="id_diametre"><?php echo $lang["id_diametre_label"]; ?></option><option value="valeur_diametre"><?php echo $lang["valeur_diametre_label"]; ?></option>
                        </select>
                    </div>
                </form>

                <h2>
                    <?php echo $lang['syno_diametre_TITLE']; ?> 
                    <sup><span class="label label-info" id="nbrOf_syno_diametre"><?php echo $nbr; ?></span>
                ...
                <div class="mail-tools tooltip-demo m-t-md">
                    ...
                    <button id="btn_new" data-toggle="modal" href="#modal-form" class="btn btn-primary"><i class="fa fa-plus-circle"></i> New syno_diametre</button>
            ...
            <div class="mail-box-header">
                <button class="btn btn-xs btn-primary" onclick="show_hide(0)">id_diametre</button> <button class="btn btn-xs btn-primary" onclick="show_hide(1)">valeur_diametre</button> 
            </div>

            <div class="mail-box">
                <table class="table table-bordered table-hover table-mail">
                    <thead>
                        <tr><th></th><th id="cell_0_0">id_diametre</th><th id="cell_0_1">valeur_diametre</th><th>Action</th></tr>
                    </thead>
                    <tbody id="syno_diametre_body_id">
                    </tbody>
                    <tfoot>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 b-r">
                        <h3 class="m-t-none m-b"><?php echo $lang["syno_diametre_modal_title"]; ?></h3>

                        <p><?php echo $lang["syno_diametre_modal_description"]; ?></p>

                        <input id="id_admin" type="hidden" value=""> 
                        
                        <div class="form-group">
                            <label for="<?php echo $lang["id_diametre_id"]; ?>"><?php echo $lang["id_diametre_label"]; ?></label> 
                            <input id="<?php echo $lang["id_diametre_id"]; ?>" placeholder="<?php echo $lang["id_diametre_placeholder"]; ?>" class="form-control " type="text" readonly>
                        </div>
                        <div class="form-group">
                            <label for="<?php echo $lang["valeur_diametre_id"]; ?>"><?php echo $lang["valeur_diametre_label"]; ?></label> 
                            <input id="<?php echo $lang["valeur_diametre_id"]; ?>" placeholder="<?php echo $lang["valeur_diametre_placeholder"]; ?>" class="form-control " type="text" >
                        </div>
                        <div>
...
<script type="text/javascript">
    ...
    var apiName = 'syno_diametre';
    var lastResponseData = null;
    var hiddenColumns = [false,false];
    
    function getData() {
        var obj = {
            <?php echo $lang["id_diametre_id"]; ?>: valById("<?php echo $lang["id_diametre_id"]; ?>"),
            <?php echo $lang["valeur_diametre_id"]; ?>: valById("<?php echo $lang["valeur_diametre_id"]; ?>"),
        };
        return obj;
    }
    ...
    function checkData(data) {
        ...
        if (data.<?php echo $lang["valeur_diametre_id"]; ?> == "") {
            showNotification("Error", ["<?php echo $lang["valeur_diametre_empty_message"]; ?>"], "error");
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.err == 0) {
                    showNotification('DONE', response.msg);
                    listAPI(page);
                    var nbr = $('#nbrOf_syno_diametre').text();
                    nbr++;
                    $('#nbrOf_syno_diametre').text(nbr);
                }
            },
            error: function (response) {},
            complete: function () {},
            ...
            success: function (response) {
                if (response.err == 0) {
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        html += '<input value="' + response.data[i].id_diametre + '" type="checkbox" class="i-checks">';
                        html += '</td>';
                        html += "<td id=\"cell_" + i + "_0\" class=\"mail-contact\">" + nullToStr(response.data[i].id_diametre) + "</td>";
                        html += "<td id=\"cell_" + i + "_1\" class=\"mail-contact\">" + nullToStr(response.data[i].valeur_diametre) + "</td>";
                        ...
                        html += '</tr>';
                        $('#syno_diametre_body_id').append(html);
                    ...
                    $('#nbrOf_syno_diametre').text('Number : ' + response.extra.NbrQ + ' Total : ' + response.extra.NbrT);
                }
            },
            error: function (response) {},
            complete: function () {},
    ...
    function btn_edit_click(index) {
        ...
        $("#<?php echo $lang["id_diametre_id"]; ?>").val(data.id_diametre);
        $("#<?php echo $lang["valeur_diametre_id"]; ?>").val(data.valeur_diametre);
