...
<div class="wrapper wrapper-content">
    <div class="row">
        <div class="col-lg-12 animated fadeInRight">
                ...
                <form class="pull-right mail-search">
                    <div class="input-group">
                        ...
                        <select id="search_by" class="form-control input-sm">
                            <option value="id_type_chambre"><?php echo $lang["id_type_chambre_label"]; ?></option><option value="lib_type_chambre"><?php echo $lang["lib_type_chambre_label"]; ?></option>
                        </select>
                    </div>
                </form>
            ...
            <div class="mail-box-header">
                <button class="btn btn-xs btn-primary" onclick="show_hide(0)">id_type_chambre</button> <button class="btn btn-xs btn-primary" onclick="show_hide(1)">lib_type_chambre</button> 
            </div>

            <div class="mail-box">
                <table class="table table-bordered table-hover table-mail">
                    <thead>
                        <tr><th></th><th id="cell_0_0">id_type_chambre</th><th id="cell_0_1">lib_type_chambre</th><th>Action</th></tr>
                    </thead>
                ...
                <div class="row">
                    <div class="col-sm-12 b-r">
                        ...
                        <div class="form-group">
                            <label for="<?php echo $lang["id_type_chambre_id"]; ?>"><?php echo $lang["id_type_chambre_label"]; ?></label> 
                            <input id="<?php echo $lang["id_type_chambre_id"]; ?>" placeholder="<?php echo $lang["id_type_chambre_placeholder"]; ?>" class="form-control " type="text" readonly>
                        </div>
                        <div class="form-group">
...
<script type="text/javascript">
    ...
    function getData() {
        var obj = {
                        <?php echo $lang["id_type_chambre_id"]; ?>: valById("<?php echo $lang["id_type_chambre_id"]; ?>"),
    ...
    function checkData(data) {
        ...
        if (data.<?php echo $lang["lib_type_chambre_id"]; ?> == "") {
            showNotification("Error", ["<?php echo $lang["lib_type_chambre_empty_message"]; ?>"], "error");
            return false;
        }
            ...
            success: function (response) {
                if (response.err == 0) {
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                        ...
                        html += '<input value="' + response.data[i].id_type_chambre + '" type="checkbox" class="i-checks">';
                        html += '</td>';
                        html += "<td id=\"cell_" + i + "_0\" class=\"mail-contact\">" + response.data[i].id_type_chambre + "</td>";
                        html += "<td id=\"cell_" + i + "_1\" class=\"mail-contact\">" + response.data[i].lib_type_chambre + "</td>";
    ...
    function btn_edit_click(index) {
        ...
        $('#btn_save').attr('data-api', 'update');
                $("#<?php echo $lang["id_type_chambre_id"]; ?>").val(data.id_type_chambre);
